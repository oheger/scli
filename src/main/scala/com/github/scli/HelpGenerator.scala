...
object HelpGenerator {
  ...
  /**
   * Returns a ''ColumnGenerator'' function that decorates another generator by
   * applying a mapping function on all the lines produced by the original
   * generator. This is an easy way to transform the output of a column
   * generator.
   *
   * @param generator the generator function to decorate
   * @param f         the mapping function on the lines
   * @return the ''ColumnGenerator'' applying the transformation function
   */
  def mapLinesColumnGenerator(generator: ColumnGenerator)(f: String => String): ColumnGenerator =
    data =>
      generator(data) map f

  /**
   * Returns a ''ColumnGenerator'' function that allows adding prefix lines to
   * the lines of another generator function. This is useful for instance to
   * add a caption to an attribute value. The prefix lines are added only if
   * the decorated generator function yields a value; otherwise, this generator
   ...
   */
  def prefixLinesColumnGenerator(generator: ColumnGenerator, prefixLines: List[String]): ColumnGenerator =
    data =>
      generator(data) match {
        case l@_ :: _ => prefixLines ++ l
        case l => l
      }

  /**
   * Returns a ''ColumnGenerator'' function that allows adding a prefix text to
   * all the lines produced by another generator function.
   *
   * @param generator  the generator function to decorate
   * @param prefixText a text to add as prefix to each original line
   * @return the ''ColumnGenerator'' generating a prefix
   */
  def prefixTextColumnGenerator(generator: ColumnGenerator, prefixText: String): ColumnGenerator =
    mapLinesColumnGenerator(generator)(prefixText + _)

  /**
   * Returns a ''ColumnGenerator'' function that allows adding a prefix to all
   * the lines produced by another generator function that is generated by a
   * third generator function. The function invokes the prefix generator. If it
   * yields data, the first line of this data is used as prefix. This prefix is
   * then added to all the lines of the original generator. Using this
   * function, the output of different generators can be combined in a single
   * cell.
   *
   * @param generator       the generator function to decorate
   * @param prefixGenerator the generator producing the prefix
   * @return the ''ColumnGenerator'' adding a prefix
   */
  def prefixGeneratedColumnGenerator(generator: ColumnGenerator, prefixGenerator: ColumnGenerator): ColumnGenerator =
    mapLinesGeneratedColumnGenerator(generator, prefixGenerator)((line, prefix) => prefix + line)

  /**
   * Returns a ''ColumnGenerator'' function that allows adding suffix lines to
   * the lines of another generator function. This is analogous to
   * ''prefixLinesColumnGenerator()'', but the lines are added after the output
   * produced by the wrapped generator (unless it is empty).
   *
   * @param generator   the generator function to decorate
   * @param suffixLines a list with lines to add as suffix to the generator
   * @return the ''ColumnGenerator'' generating a suffix
   */
  def suffixLinesColumnGenerator(generator: ColumnGenerator, suffixLines: List[String]): ColumnGenerator =
    data =>
      generator(data) match {
        case l@_ :: _ => l ++ suffixLines
        case l => l
      }

  /**
   * Returns a ''ColumnGenerator'' function that allows adding a suffix text to
   * all the lines produced by another generator function.
   *
   * @param generator  the generator function to decorate
   * @param suffixText a text to add as suffix to each original line
   * @return the ''ColumnGenerator'' generating a suffix
   */
  def suffixTextColumnGenerator(generator: ColumnGenerator, suffixText: String): ColumnGenerator =
    mapLinesColumnGenerator(generator)(_ + suffixText)

  /**
   * Returns a ''ColumnGenerator'' function that allows adding a suffix to all
   * the lines produced by another generator function that is generated by a
   * third generator function. This is analogous to
   * ''prefixGeneratedColumnGenerator()'', but the output of the suffix
   * generator is appended to the lines of the wrapped generator.
   *
   * @param generator       the generator function to decorate
   * @param suffixGenerator the generator producing the suffix
   * @return the ''ColumnGenerator'' adding a suffix
   */
  def suffixGeneratedColumnGenerator(generator: ColumnGenerator, suffixGenerator: ColumnGenerator): ColumnGenerator =
    mapLinesGeneratedColumnGenerator(generator, suffixGenerator)((line, suffix) => line + suffix)
  ...
  /**
   * Returns a ''ColumnGenerator'' function that allows modifying the output of
   * a wrapped generator based on the result produced by another generator. The
   * first line of the output of the modifier generator (if defined) is
   * obtained. Then, on each line of the original generator, the mapping
   * function is called with the original line and the modifier as input.
   *
   * @param generator    the ''ColumnGenerator'' to modify
   * @param modGenerator the ''ColumnGenerator'' producing the modifier
   * @param f            the mapping function
   * @return the modifying ''ColumnGenerator''
   */
  private def mapLinesGeneratedColumnGenerator(generator: ColumnGenerator, modGenerator: ColumnGenerator)
                                              (f: (String, String) => String): ColumnGenerator =
    data => {
      generator(data) match {
        case l@_ :: _ =>
          val optModifier = modGenerator(data).headOption
          optModifier.fold[List[String]](l)(modifier => l map (line => f(line, modifier)))
        case l => l
      }
    }
