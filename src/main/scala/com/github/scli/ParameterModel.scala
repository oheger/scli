...
object ParameterModel {
  ...
  /**
   * The attribute containing the original value passed on the command line
   * that caused a transformation failure. Note that this attribute is only
   * present for failures if the failure is caused by a failed transformation,
   * and the original value could be determined.
   */
  final val AttrErrOriginalValue = ParameterAttributeKey[String]("errOriginalValue")
  ...
  /**
   * A class storing information about failures that were detected during
   * parameter processing.
   *
   * The main purpose of this class is to store failure information and to
   * transform it into [[ParameterMetaData]] objects. This makes the
   * information accessible in the same way as parameter metadata used for the
   * generation of help screens.
   *
   * The metadata produced by this class is similar to the one generated by
   * [[ModelContext]], except for the following differences:
   *
   *  - The alias list contains always a single alias, which is the
   *    ''failureKey'' from the failure information.
   *  - The attributes in the metadata contain additional information related
   *    to failures; there are corresponding constants for these attributes
   *    starting with the prefix ''AttrErr''.
   *
   * @param modelContext the ''ModelContext''
   * @param failures     a collection with all failures that occurred
   */
  class FailureContext(val modelContext: ModelContext,
                       val failures: Iterable[ParameterFailure]) extends ParameterMetaDataSource {
    /**
     * Stores the metadata that has been constructed based on the failures and
     * the model context passed to this instance.
     */
    override val parameterMetaData: Iterable[ParameterMetaData] = createParameterData()

    /**
     * Converts the failures passed to this object to corresponding metadata
     * objects as described in the class's comment. The list of failures may
     * contain duplicates, e.g. if an extractor has been executed multiple
     * times; these are filtered out.
     *
     * @return the collection with ''ParameterMetaData''
     */
    private def createParameterData(): Iterable[ParameterMetaData] =
      failures.foldRight((List.empty[ParameterMetaData], Set.empty[(ParameterKey, String)])) { (failure, t) =>
        val errDesc = (failure.failureKey, failure.cause.toString)
        if (t._2 contains errDesc) t
        else (convertFailure(failure) :: t._1, t._2 + errDesc)
      }._1

    /**
     * Converts a failure to a ''ParameterMetaData'' structure.
     *
     * @param failure the failure
     * @return the resulting ''ParameterMetaData''
     */
    private def convertFailure(failure: ParameterFailure): ParameterMetaData = {
      val aliases = List(failure.failureKey)
