...
* If none of the configured switches is present on the command line, result is
  `Success(None)`.
* If the user specified multiple of the switches that exclude each other,
  result is a `Failure`; the failure message points to the keys of switches
  that are not allowed together.
...
applies the exclusion criteria:
...
mapping function to produce the final result type. The function checks whether
at most one of the configured extractors yields a defined value. If so, it
invokes the mapping function with the value and the key of the producing
extractor (or `None` if no value at all is found). If there are multiple
values, however, it produces a failure.
