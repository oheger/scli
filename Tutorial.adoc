...
`SingleOptionValue[String]` type; per default, it fails if the option occurs
...
Above we stated that the `optionValue()` extractor "fails if the option occurs
multiple times". This seems quite natural, but there is one exception to this
rule: `optionValue()` accepts an additional Boolean parameter named
`allowOverride`. If set to *true*, the extractor does not fail if it encounters
multiple option values, but takes only the last value ignoring any previous
ones. This basically means that options occurring later on the command line can
override options before. A use case where this behaviour if useful is
extracting standard options to <<Reading parameter files,parameter files>>: The
parameter file can contain a default value for a specific option. It is,
however, still possible to override this value by simply adding the option
again to the command line (after the option that includes the file) and setting
the new value.
...
the _optionValues()_ function is responsible. Its signature is analogous to
the one of the _optionValue()_ function:

[source,scala]
----
val extLog = optionValues("log")
----

Except for the different result type, extractors created by the functions
_optionValue()_ and _optionValues()_ are very similar in the features they
...
for multi option values (created by `optionValues()`): `mandatory` only
...
pair of functions analogously to `optionValue()`, and `optionValues()`:
...
==== Switches excluding each other

Related to extractors executed under certain conditional logic, are extractors
that exclude each other. This can be viewed as a kind of condition, too: A
specific option can only be specified on the command line if a certain other
option is absent, as they contradict each other.

One example for such a constellation can be switches that define the log level
of an application. The application may be able to produce logging output in the
levels _debug_, _info_, _warn_, and _error_. It may offer the user
corresponding switches to select a specific log level. So, for instance, by
specifying the `--info` switch, the log level would be set to _info_. In this
context, it would make no sense to specify multiple of these switches; what
should be printed if the user specified both the `--error` and `--debug`
switches?

SCLI offers a function named `excludingSwitches()` to address exactly this use
case. The function expects an arbitrary number of extractors of type
`Try[Boolean]`, i.e. the data type used by switches. It returns an extractor of
type `SingleOptionValue[String]`. This can take the following values:

* If the user has specified exactly one of the switch options configured for
  `excludingSwitches()`, result is a `Success` with a defined `Option` with the
  key of the selected switch. (In the example with the switches for log levels,
  this could be for instance `Success(Some("info"))`, or
  `Success(Some("debug"))`.)
* If none of the configured switches is present on the command line, result is
  `Success(None)`.
* If the user specified multiple of the switches that exclude each other, the
  result depends on the additional _allowOverride_ parameter accepted by
  `excludingSwitches()`, which is analogous to the equally named parameter
  described under <<Extracting a single string option value>>:
  - If this flag is *false*, result is a `Failure`; so this is actually a hard
    exclusion. The failure message points to the keys of switches that are not
    allowed together.
  - If this flag is *true*, the result corresponds to the switch that occurs at
    the latest position on the command line; so this switch overrides the
    others.

The _transfer_ demo application implements exactly this use case with switches
for log levels. Here is the code that defines the different switches and
applies the exclusion criteria; note that overrides are allowed:
...
When extracting a `TransferConfig` from the command line options, the
_extLogLevel_ extractor is used, rather than the actual log level switches. It
yields the selected log level as a string - one of "debug", "info", "warn", or
"error". If the user has not explicitly set a log level, it uses "warn" as a
fallback value.

Switches contradicting each other is only one example for excluding command
line options. SCLI therefore offers a generic function, `excluding()` to deal
with this topic. (In fact, `excludingSwitches()` is implemented on top of this
function.) `excluding()` expects an arbitrary number of extractors of type
`SingleOptionValue[A]` for a generic type `A`. In addition, it requires a
mapping function to produce the final result type. The _allowOverride_ flag is
supported as well; however, overriding works only if all the excluding
extractors passed to the function can be associated with specific elements on
the command line. This is the case for primitive extractors, but not
necessarily for complex ones with conditional logic or that combine the values
of multiple other options. In this case, the concept of a last option on the
command line is undefined; therefore, an override is not possible.

The generic `excluding()` function checks whether at most one of the configured
extractors yields a defined value or if overriding is enabled and can be
applied. If so, it invokes the mapping function with the selected value and the
key of the producing extractor (or `None` if no value at all is found). Result
is again a failure, if there are multiple values and _allowOverride_ is
*false* or overriding is not possible.
...
==== Customizing transformation exceptions

In addition to the problems detected by the `ParameterExtractor` module - as
discussed in the previous subsection -, there is another source of potential
errors: transformations on extracted values. Take for instance the `toInt()`
modifier. The function tries to convert the value entered by the user to a
number. If this is not possible, the transformation causes a
`NumberFormatException` (which is a standard Java exception describing this
conversion problem). Other converter functions behave similarly; they use
standard Java functionality to do their specific conversion, and thus can
produce standard Java exceptions. Developers can write custom mapping functions
to do custom conversions; those functions may throw arbitrary exceptions.

The `ParameterExtractor` module deals with such cases by wrapping invocations
of mapping functions into `Try` instances. If an invocation fails, the causing
exception can be extracted from the `Try` and stored in the failure context.
The error report then shows the messages of these exceptions.

This is a working approach, but sometimes the exception messages are rather
technical or do not describe the actual problem in a way that can be understood
by the target audience. The message of a `NumberFormatException` is an example
of such a case; in Java 11 it just contains the text _For input XXX_, where
_XXX_ is replaced by the invalid value entered by the user. This message is
meaningful only if the error report shows the exception type; but then it
already becomes pretty technical.

SCLI addresses the problem by supporting an `ExceptionMapper` function. Its
...
The function expects the key of the parameter affected by the exception and an
`Option` with a `CliElement`. `CliElement` holds information about the
parameter value as it has been entered by the user; namely the parameter key
and the original value. The function returns a `PartialFunction`, which does
the actual exception mapping; here you can match for specific exceptions and
return alternative ones.

The _transfer_ demo application defines a custom exception mapper function,
which implements the following mappings:

* `NumberFormatException` exceptions are mapped to `IllegalArgumentException`
  exceptions with a better exception message. (As the error report of the
  application does not display the exception type, the actual exception class
  is rather irrelevant; only the message is important.)
* If the user enters an invalid encryption mode, the mapping function generates
  a more concrete error message. This conversion shows how to evaluate the
  parameter key in the mapping function.

The code fragment, which defines the exception mapper function, is shown below:
...
NOTE: The `CliElement` passed as second parameter to the mapping function is an
`Option` because there is no absolute guarantee that the problematic parameter
value can be associated with a specific element on the command line under all
circumstances. For instance, an application may apply a mapping function to an
extractor that filters out some values entered for a parameter. In such cases,
the `Option` parameter is undefined. As the demo application does not do such
things, it relies on the presence of this information and just calls `get` on
the `Option` instance, although this is certainly no good practice in general.

The exception mapper function is activated in the same way as a custom
exception generator function: by setting a property of the `ExtractionSpec`
object passed to `ParameterManager`:

 val spec = ExtractionSpec(transferCommandConfigExtractor,
   ...
   exceptionMapper = exceptionMapper)

We can test the special exception mapper function by passing invalid values to
the numeric `chunk-size` and the `crypt-mode` parameters:

 transfer upload test.txt /data --chunk-size large --crypt-mode secret

This is the error report generated for this input:

[source]
----
Invalid parameters have been detected:

--chunk-size  Input cannot be converted to a number: "large"

--crypt-mode  This is not a valid encryption mode: "secret"
----

==== Error handling in transformation functions

The previous subsection already gave an explanation how SCLI deals with
exceptions thrown by transformations on `CliExtractor` objects. Basically, such
exceptions are caught and added to the failure context, which is the source for
error reports.

This is applicable to custom mapping functions as well. Your mapping logic can
throw arbitrary exceptions. In this scenario, you have typically control over
the concrete exceptions; so you can directly generate messages suitable for the
error report to the end user - and thus can skip an additional step with an
exception mapper function.

Let us have a look at an example from the _transfer_ demo application. The
application supports an option `--attribute` (which can be abbreviated using
the alias `-A`) to add a metadata attribute to the current transfer operation.
The option can occur multiple times to set arbitrary attributes. The value of
the option needs to be of the form _<key>=<value>_. The corresponding extractor
therefore checks whether the value conforms to this pattern. If this is the
case, it can transform the value to an `Attribute` object; otherwise, an
exception is thrown to report the invalid option value. Here are the pieces of
code related to this transformation:
...
The declaration of the `CliExtractor` for the `attribute` option uses the
`mapTo()` modifier to apply a transformation function - here `toAttribute()` -
to the original string value. `toAttribute()` checks the string against a
regular expression that splits it into a key/value pair. With this information,
the `Attribute` instance can be constructed. If the regular expression does not
match, however, the function throws an exception with a meaningful message. As
has already been mentioned, the concrete exception type does not really matter
for the demo application because it is not shown in the error report. If you
prefer, you can of course use specialized exception types.

An invocation of the demo application with invalid attribute options like the
following one:

 transfer upload test.txt /data --attribute stage=qa \
   --attribute invalid1 -A invalid2
...
--attribute  Invalid attribute value: "invalid1" - attributes must have
             the form <key>=<value>.

-A           Invalid attribute value: "invalid2" - attributes must have
             the form <key>=<value>.
----
